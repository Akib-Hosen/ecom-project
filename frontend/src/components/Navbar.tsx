        ...
        <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
                ...
                <div className="shrink-0">
                    <Link href={user?.role === "seller" ? "/seller/dashboard" : "/"}>
                        <img
                            src="/logo.png"
                            alt="bdMart"
                            ...
                            <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-100 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                <ul className="py-2">
                                    ...
                                    {categories.map((category: any) => (
                                        <li key={category.id}>
                                            <Link href={`/products?category=${category.name}`} className="block px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-[#0047FF] transition-colors capitalize">
                                                {category.name}
                                            </Link>
                                        </li>
                                    ))}
